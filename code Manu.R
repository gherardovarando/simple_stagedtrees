...
library(arules)
library(deal)
setwd("C:/Users/Manuele.Leonelli/Desktop")
...
# BodyFat
#load("C:/Users/Manuele.Leonelli/Desktop/BodyFat.RData")
#data <- data[,-1]
#dag_bf <- hc(data)
#graphviz.plot(dag_bf)
#order <- c("Weight","Abdomen","Biceps","Forearm","Knee","Neck","Ankle","Age","Wrist","Chest","Hip","Thigh","BodyFat","Height")

#simple_bf <- simple_marginal(data,  order = order)
#bhc_bf <- stages_bhc(full(data,  order = order, join_unobserved = F,lambda = 1 ))
#simplified_bf <- simplify(bhc_bf)
#total_bf <- simple_total_bhc(full(data,  order = order, join_unobserved = F, lambda =1))

#bic <- rbind(bic,c(-2*BIC(dag_bf,data),BIC(simple_bf),BIC(simplified_bf),BIC(total_bf),BIC(bhc_bf)))
#pos <- rbind(pos,c(num_positions(simple_bf),num_positions(simplified_bf),num_positions(total_bf),num_positions(bhc_bf),num_stages(bhc_bf)/num_positions(bhc_bf)))
