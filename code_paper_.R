...
simple_marginal_bhc <- function(data, lambda = 1, order = NULL){
  if (is.null(order)){
    if (is.table(data)){
...
simple_total_bhc <- function (data, lambda = 1, order = NULL, max_iter = Inf) {
  if (is.null(order)){
    if (is.table(data)){
      order <- names(dimnames(data))
    }else if(is.data.frame(data)){
  ...
  score <- function(x){ -BIC(x)}
  object <- full(data, order  = order, lambda = lambda, join_unobserved = FALSE)
...
simple_cor <- simple_marginal_bhc(coronary, order = c("S","PW","PRE","MW","F","PRO"))
bhc_cor <- stages_bhc(full(coronary,order = c("S","PW","PRE","MW","F","PRO"), join_unobserved = F,lambda = 1 ))
simplified_cor <- simplify(bhc_cor)
total_cor <- simple_total_bhc(coronary, order = c("S","PW","PRE","MW","F","PRO"))
...
simple_chds <- simple_marginal_bhc(chds,  order = c("Social","Economic","Events","Admission"))
...
total_chds <- simple_total_bhc(chds, order = c("Social","Economic","Events","Admission"))
...
simple_phd <- simple_marginal_bhc(PhDArticles,  order = c("Articles","Mentor","Prestige","Gender", "Kids","Married"))
...
total_phd <- simple_total_bhc(PhDArticles,  order = c("Articles","Mentor","Prestige","Gender", "Kids","Married"))
...
simple_tit <- simple_marginal_bhc(titanic.df,  order = c("Class","Sex","Survived","Age"))
...
total_tit <- simple_total_bhc(titanic.df,  order = c("Class","Sex","Survived","Age"))
